...
covered as a public contract with cross-language BLAKE3 `unique_key`, domain-separated
`concurrency_key`,
...
| **#342** SQL producer contract | P1 | Introduce `awa.insert_job` as the single-signature public capability (D6), backed by internal compatibility/storage helpers: documented request/result/error semantics versioned against `schema_version`; cross-language test vectors for BLAKE3 `unique_key`, the domain-separated `concurrency_key` digest, and portable shard routing with ADR-033 precedence (`ordering_key`, else `concurrency_key`, else rotor); a conformance script any producer implementation can run against a live schema; `docs/sql-producer-contract.md`. Reference snippets (Node/Go) as *examples*, explicitly unsupported — no official third-language clients (issue's own out-of-scope). |
