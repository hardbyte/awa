...
append-only execution grants at claim time, shipped only if E5 finds a design with **zero new hot
mutable rows** and bounded claim-path cost.
...
| **#340** Per-key control — Tier 2 | P2 (E5-gated) | **ADR-033.** Fleet-exact concurrency via a database-authoritative policy plus append-only execution grants/closures. Concurrent claim decisions serialize on a transaction-scoped key advisory lock; the allocator admits only a contiguous FIFO prefix and never advances over a gated head. Ships only if E5 shows acceptable claim-path cost, no hot mutable counter family, and no cursor-monotonicity violation. Cross-key fairness is a separate allocator decision. |
| **#401** Caller-owned finalization | P1 | **ADR-042.** Rust `complete_in_tx` plus a versioned worker-only SQL completion contract let application rows and completion of the exact run lease commit together. A durable `FinalizationReceipt` is reconciled before permits/key grants are released; stale finalization aborts the caller transaction. Completion-only v1, queue-storage only, opt-in single-job slow path; explicit in-transaction follow-ups replace process-local Completed specs for participating kinds. |
...
(a) worker-local token bucket (Tier 1 baseline), (b) claim-time append-only grants serialized by
transaction-scoped key locks, (c) shard-pinning + per-shard caps. Skewed tenant distributions
(Zipf); metrics: fairness
