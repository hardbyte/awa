...
| **#143** `awa-api` crate split | P0 | Extract API response types + handlers from `awa-ui`; `JobResponse`-style wrappers everywhere (no `FromRow` structs over HTTP); `awa-cli` depends on `awa-api` without the React build. Prerequisite for D6's HTTP-API stability promise and for #368 health surfaces. |
...
**M0 — Foundations.**
#335, #360, #367, #143, #368, #369, #370 groundwork, housekeeping closes, #383 tracker
opened with §7 gates.
...
#282, #372 (ADR-027/028 → Accepted), #118, #343, #373, #374, #344 (last — consumes all of
the above).
...
**M3 — Observability.**
#110 (Rust path → Python boundary), E8 gate, #375, #376.
...
New issues filed: **#367–#382** (§3) plus **#383** (release tracker). ADR numbers 033–037
are claimed by placeholder files in `docs/adr/` per the numbering convention.
