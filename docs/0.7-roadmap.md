...
to a semver promise: Rust API, Python API, the SQL producer contract (#342 — `insert_job`
covered as a public contract with cross-language BLAKE3 `unique_key`, domain-separated `concurrency_key`,
and shard-routing test vectors), the ADR-042 SQL worker finalization contract
(`complete_job` receipt, error, and version semantics), the
...
| **#342** SQL producer contract | P1 | Introduce `awa.insert_job` as the single-signature public v1 capability (D6), backed by internal compatibility/storage helpers: documented request/result/error semantics versioned against `schema_version`; cross-language test vectors for BLAKE3 `unique_key`, the domain-separated `concurrency_key` digest, and portable shard routing with ADR-033 precedence (`ordering_key`, else `concurrency_key`, else rotor); a conformance script any producer implementation can run against a live schema; `docs/sql-producer-contract.md`. Reference snippets (Node/Go) as *examples*, explicitly unsupported — no official third-language clients (issue's own out-of-scope). |
...
- **Strict PostgreSQL capability roles** (#452 / ADR-043). ADR-042 ships its narrow finalizer
  boundary; replacing the ordinary runtime's blanket table/function grants is a later
  expand/use/tighten program with a direct-COPY performance decision.
...
| #452 PostgreSQL capability boundary | ADR-043 design only; implementation deferred beyond 0.7. |
