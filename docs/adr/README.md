...
| 033 | [Per-key execution control](033-per-key-execution-control.md) | Proposed | Worker-local approximation plus fleet-exact append-only execution grants serialized at claim time; fairness remains a separate allocator problem (#340) | Composes ADR-005/010/011/025/031; bound by ADR-023/026's ledger discipline; caller completion per ADR-042 |
...
| 041 | [Rolling upgrade policy](041-rolling-upgrade-policy.md) | Accepted | Rolling upgrades use expand → capability-gated flip → later contract; version floors guard expand migrations, while database fences and real N-1 rehearsals guard irreversible flips. | Generalizes ADR-040; strengthens ADR-036; checklist in `docs/development.md` |
| 042 | [Caller-owned finalization transactions](042-caller-owned-finalization-transactions.md) | Proposed | Application rows and guarded completion of the exact run lease commit in one caller-owned Postgres transaction, with durable receipt reconciliation (#401) | Extends ADR-013/029; composes with ADR-033 exact key-grant closure and #342's SQL contract |
