...
| 040 | [Append-only ring-rotation ledgers](040-append-only-ring-rotation-ledger.md) | Accepted | Ring cursors move from mutable `{ring}_ring_state` singletons to append-only `{ring}_ring_rotations` ledgers (cursor = max-generation row; CAS on the generation PK); per-slot generations derived; rotate/prune serialize on a per-ring advisory lock; queue prune appends `queue_terminal_rollup_deltas` folded by horizon-gated maintenance; loud-failure compat break (v042, lockstep upgrade, #371) | Extends ADR-023's ring plane; applies ADR-026's dead-tuple reclaim discipline to the ring control plane; modelled in AwaStorageLockOrder / AwaDeadTupleContract |
