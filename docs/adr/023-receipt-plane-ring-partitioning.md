...
- Claim: append queue-slot-local batch/range attempt evidence to `ready_claim_attempt_batches` and append receipts to the current `lease_claims` child partition in the same transaction. The claim result carries `claim_slot` through to the worker so closure evidence can be tied back to the matching claim partition.
...
- Short-job fast path moves the cost to append-only ledgers: claim writes receipt rows plus one ready-slot attempt batch row for the claimed lane range, and compact success writes batch-level terminal/count evidence without a per-job closure row or receipt delete.
