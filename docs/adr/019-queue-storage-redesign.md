...
- prune-claim-ring (added by ADR-023) takes `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on the target `claim_ring_slots` row, and bounded `ACCESS EXCLUSIVE` on the matching `lease_claims_*`, `lease_claim_closures_*`, and `lease_claim_closure_batches_*` partitions, verifies every claim in the slot has durable closure evidence, then `TRUNCATE`s those partitions in lockstep. Open claims make prune skip the slot until normal completion, normal non-success closure, or the separate receipt-rescue scans close them.
