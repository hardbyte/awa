...
For successful receipt-backed completions that do not need wide terminal body fields, write compact rows to `receipt_completion_batches` instead of one `done_entries` row per completed job. Each compact terminal batch stores arrays of `(job_id, run_lease, lane_seq, attempt, attempted_at)` plus the shared ready segment, queue, priority, shard, finalized timestamp, and originating claim slot. The same completion transaction also writes a compact claim-local row to `lease_claim_closure_batches`, keyed by immutable, globally allocated `lease_claims.receipt_id` values and a derived `int8multirange` for indexed membership checks. The runtime routes claim lookup and compact closure writes to the originating `claim_slot` child partitions, while compact terminal history and terminal-count deltas remain queue-slot routed. The compact closure batch carries the ready slot and generation for the completed group, so queue and claim prune can first compare exact row counts for a sealed segment and only fall back to per-claim membership checks when counts do not prove closure. Terminal history and claim-closure proof are therefore separate append-only ledgers, so successful completions do not need per-job `lease_claim_closures` rows and stale receipt scans do not need to unnest terminal-history arrays.
