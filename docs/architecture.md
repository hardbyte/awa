...
| `lease_claims_*` / `lease_claim_closures_*` / `lease_claim_closure_batches_*` | Receipt-plane execution history for short attempts. | Internal storage. Live receipt attempts are claims without durable closure evidence. Explicit closures cover non-success and cold paths; compact closure batches cover successful hot-path completions and expose a range-indexed receipt membership proof. Maintenance uses `claim_ring_slots` rescue cursors to keep stale and deadline scans bounded when closed receipt history cannot yet be truncated. |
