...
| **SQL producer contract** | `awa.insert_job(...)` per [#342](https://github.com/hardbyte/awa/issues/342): one exact signature, semantics, BLAKE3 `unique_key` derivation, domain-separated `concurrency_key` digest, and shard precedence/vectors (`ordering_key`, else `concurrency_key`, else rotor). Versioned against `awa.schema_version`; conformance script provided. | `insert_job_compat`, every other internal function, and direct DML against storage tables. |
| **SQL worker finalization contract** | `<queue_storage_schema>.complete_job(...)` per ADR-042: one exact signature, token arguments, portable `FinalizationReceipt` result, stale-token SQLSTATE that aborts the transaction, and guarded completion semantics. Versioned against `awa.schema_version`; breaking evolution follows ADR-036 deprecation and ADR-041 expand/migrate/contract rather than silently changing under live callers. | Internal completion helpers and physical receipt/lease/terminal representations. Direct DML and savepoint containment that commits after a stale-token error are unsupported. |
