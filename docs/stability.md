...
| **SQL producer contract** | `awa.insert_job_compat(...)` per [#342](https://github.com/hardbyte/awa/issues/342): signature, semantics, BLAKE3 `unique_key` derivation, domain-separated `concurrency_key` digest, and shard precedence/vectors (`ordering_key`, else `concurrency_key`, else rotor). Versioned against `awa.schema_version`; conformance script provided. | Every other function, table, view, and trigger in the `awa.*` schema. Direct DML against storage tables is unsupported. |
| **SQL worker finalization contract** | `complete_job_compat(...)` per ADR-042: exact token arguments, portable `FinalizationReceipt` result, stale-token SQLSTATE that aborts the transaction, and guarded completion semantics. The function is the canonical versioned entry point and is versioned against `awa.schema_version`; breaking changes use a new name and the ADR-036 deprecation window. | Internal completion helpers and physical receipt/lease/terminal representations. Direct DML and savepoint containment that commits after a stale-token error are unsupported. |
