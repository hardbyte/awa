-- Schema patch wait_free_dirty_marks: wait-free admin dirty-key marks (#492)
-- Same SQL as awa 0.7 migration v046, minus the schema_version row.
--
...
-- transitions. The marks become append-only rows in tables with no index or
-- constraint, so the trigger INSERT never waits; the drain deletes the visible
-- rows it recounts, and the refresh deletes instead of truncating.
--
-- 0.6 delivery: the 0.6 series cannot add a migration version (0.7 owns v041
-- and above), so this ships as an idempotent schema patch. `awa migrate`
-- applies it when the schema is exactly at v040 and the probe in
-- migrations.rs reports it missing; `awa migrate --sql` prints it as a
-- repeatable R__ script for external runners. Released 0.6 binaries call the
-- maintenance functions by name and operate unchanged once it is applied. The
-- old admin_dirty_queues / admin_dirty_kinds tables stay in place, unread. No
-- lock here conflicts with job traffic; the patch completes in milliseconds.
