...
async fn queue_prune_has_unclosed_claim_refs_tx(
    ...
    let count_proves_claim_refs_closed: bool = sqlx::query_scalar(audited_sql(format!(
        r#"
        WITH claim_count AS (
...
async fn claim_prune_has_open_claims_tx(
    ...
    let count_proves_claims_closed: bool = sqlx::query_scalar(audited_sql(format!(
        r#"
        WITH claim_count AS (
...
impl QueueStorage {
    ...
    async fn ensure_lane<'a>(
        ...
        if self.lane_is_cached(queue, priority, enqueue_shard) {
            let schema = self.schema();
            let visible: bool = sqlx::query_scalar(audited_sql(format!(
                r#"
                SELECT EXISTS (
                    SELECT 1
    ...
    async fn advance_enqueue_head<'a>(
        ...
        self.ensure_lane_inserts(tx, queue, priority, enqueue_shard)
            .await?;
        let start: i64 = sqlx::query_scalar(audited_sql(sql))
            .bind(queue)
            .bind(priority)
            .bind(enqueue_shard)
            .bind(count)
    ...
    async fn append_ring_rotation_tx<'a>(
        ...
        match authority {
            ...
            RingAuthority::Columns => {
                ...
                let advanced = sqlx::query(audited_sql(format!(
                    "UPDATE {schema}.{state} \
    ...
    async fn next_job_ids<'a>(
        ...
        sqlx::query_scalar(audited_sql(query))
            .bind(count as i32)
            .fetch_all(tx.as_mut())
            .await
            .map_err(map_sqlx_error)
    ...
    async fn complete_receipt_runtime_batch_fast(
        ...
        for (claim_slot_index, group) in by_partition {
            ...
            let completed: Vec<(i64, i64)> = match sqlx::query_as(audited_sql(format!(
                r#"
                WITH completed(
                    claim_slot,
                    ready_slot,
    ...
    async fn queue_counts_exact(
        ...
        let row: (i64, i64, i64, i64) = sqlx::query_as(audited_sql(format!(
            r#"
            WITH lane_counts AS (
    ...
    pub async fn queue_counts_fast(
        ...
        let running: i64 = sqlx::query_scalar(audited_sql(format!(
            r#"
        ...
        // column.
        let (pruned_completed, pruned_failed): (i64, i64) = sqlx::query_as(audited_sql(format!(
            r#"
            SELECT
                COALESCE(sum(
                    GREATEST(
    ...
    pub async fn pruned_failed_count_for_queue(
        ...
        let pruned_failed: i64 = sqlx::query_scalar(audited_sql(format!(
            r#"
            SELECT
                COALESCE((
    ...
    async fn update_deferred_batch_fields_tx<'a>(
        ...
        let schema = self.schema();
        let row: Option<DeferredJobRow> = sqlx::query_as(audited_sql(format!(
            r#"
            SELECT
                job_id,
                kind,
    ...
    async fn oldest_initialized_claim_slot_tx<'a>(
        ...
        let schema = self.schema();
        let preferred_slot = sqlx::query_as::<_, (i32, i64, i32)>(audited_sql(format!(
            r#"
    ...
    pub async fn load_job(&self, pool: &PgPool, job_id: i64) -> Result<Option<JobRow>, AwaError> {
        ...
        let lease_claim_batch_rows: Vec<LeaseJobRow> = sqlx::query_as(audited_sql(format!(
            r#"
            SELECT
                claim_batches.ready_slot,
                claim_batches.ready_generation,
    ...
    async fn relation_has_rows_tx(
        ...
        sqlx::query_scalar(audited_sql(format!(
            "SELECT EXISTS (SELECT 1 FROM {relation} LIMIT 1)"
        )))
        .fetch_one(tx.as_mut())
        .await
        .map_err(map_sqlx_error)
    ...
    pub async fn rotate(&self, pool: &PgPool) -> Result<RotateOutcome, AwaError> {
        ...
        // current slot. The ledger append below would otherwise only grow
        // the `queue_ring_rotations` ledger with a row that carries no
        // sealed data (and the folder cannot trim it while an MVCC horizon
        // is pinned).
        //
    ...
    pub async fn rotate_leases(&self, pool: &PgPool) -> Result<RotateOutcome, AwaError> {
        ...
        // empty (parent probe, all partitions). Otherwise the ledger
        // append below only grows the `lease_ring_rotations` ledger (the
        // lease ring was the top residual pinned-horizon dead-tuple
        // accumulator at the historical 250ms singleton-UPDATE cadence —
        // ~14.4k dead tuples/hour). See `rotate` for both the race
        // analysis and why the whole-ring scope is load-bearing (prune
        // never advances slot generations; freezing rotation with sealed
        // rows behind would strand them until traffic resumed).
    ...
    pub async fn terminal_counter_trusted(&self, pool: &PgPool) -> Result<bool, AwaError> {
        let schema = self.schema();
        let trusted: Option<bool> = sqlx::query_scalar(audited_sql(format!(
    ...
    async fn terminal_delta_rollup_candidates(
        ...
        for (slot, generation) in sealed_slots {
            ...
            let has_pending: bool = sqlx::query_scalar(audited_sql(format!(
                r#"
                SELECT EXISTS (
                    SELECT 1
                    FROM {delta_child}
    ...
    async fn rollup_terminal_count_delta_slot(
        ...
        let pending_ready: bool = sqlx::query_scalar(audited_sql(format!(
            r#"
            WITH claim_cursors AS MATERIALIZED (
                SELECT
                    queue,
        ...
        let lock_delta = sqlx::query(audited_sql(format!(
        ...
        let delta_rows: i64 = sqlx::query_scalar(audited_sql(format!(
            r#"
            SELECT count(*)::bigint
            FROM {delta_child}
            WHERE ready_generation = $1
        ...
        let grouped_keys: i64 = sqlx::query_scalar(audited_sql(format!(
            r#"
        ...
        let truncate_delta = sqlx::query(audited_sql(format!("TRUNCATE TABLE {delta_child}")))
            .execute(tx.as_mut())
            .await;
    ...
    pub async fn fold_terminal_rollup_deltas(
        ...
        let deltas: Vec<(String, i16, i64, i64)> = sqlx::query_as(audited_sql(format!(
            r#"
    ...
    pub async fn fold_ring_rotation_ledgers(
        ...
        for family in [RingFamily::Queue, RingFamily::Lease, RingFamily::Claim] {
            let ledger = family.ledger_relname();
            let state = family.state_relname();
            let deleted = sqlx::query(audited_sql(format!(
                r#"
    ...
    pub async fn prune_oldest(
        ...
        let carried_failed_rows = if retention_floor {
            ...
            let carried = sqlx::query(audited_sql(format!(
                r#"
        ...
        let pruned_terminal_counts: Vec<(String, i16, i64, i64)> =
            sqlx::query_as(audited_sql(format!(
                r#"
            WITH done_counts AS (
                SELECT
    ...
    pub async fn rotate_claims(&self, pool: &PgPool) -> Result<RotateOutcome, AwaError> {
        ...
        // empty (parent probes, all partitions). Otherwise the ledger
        // append below only grows the `claim_ring_rotations` ledger (the
        // pre-ledger singleton UPDATE accumulated ~3.6k dead tuples/hour
        // under a pinned MVCC horizon at the historical cadence). See
        // `rotate` for both the race analysis and why the whole-ring scope
        // is load-bearing (prune never advances slot generations; freezing
        // rotation with sealed receipt evidence behind would strand it
        // until traffic resumed).
        let ring_parents = [
