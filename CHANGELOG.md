...
- **Partitioned queue helper** ([#327](https://github.com/hardbyte/awa/pull/327)). Rust `PartitionedQueue` + `ClientBuilder::partitioned_queue` and Python `awa.PartitionedQueue` for deterministic routing from one hot logical queue to multiple physical queues; duplicate physical declarations are rejected.
