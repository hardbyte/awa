...
The 0.6 line makes the append-only **queue-storage engine** the default and the supported substrate for high-throughput, low-bloat operation on managed Postgres. The [#169](https://github.com/hardbyte/awa/issues/169) pinned-MVCC dead-tuple gate **passed** on `main`: a 60-minute idle-in-transaction soak holds bounded queue depth through the pinned hour and drains fully in recovery, after the last dominant hot-row update path (the `queue_claim_heads` ready-segment routing cache) was removed in the rc line. See "Benchmark evidence" below for the caveats — awa is not immune to long readers, and this release does not claim otherwise.
...
- **No remaining hot-row update paths on the claim/complete loop.** Lane cursors moved from MVCC-updated rows to Postgres sequences ([#321](https://github.com/hardbyte/awa/pull/321), v027); completion/DLQ/retry/discard stopped `DELETE FROM ready_entries` in favour of append-only ready segments plus a tombstone ledger ([#323](https://github.com/hardbyte/awa/pull/323), v028); terminal counts append signed rows to a delta ledger folded asynchronously by the maintenance leader ([#329](https://github.com/hardbyte/awa/pull/329), v030, [ADR-026](docs/adr/026-narrow-terminal-history.md)); and the per-claim `queue_claim_heads` routing-cache update — the last dominant dead-tuple accumulator under a pinned horizon — was removed by routing claim through the `ready_segments` control plane ([#355](https://github.com/hardbyte/awa/pull/355), v039). Rollup mutation and segment truncation stand down while another backend pins the MVCC horizon, then catch up once it clears ([#333](https://github.com/hardbyte/awa/pull/333)).
...
- **Maintenance branch observability** ([#302](https://github.com/hardbyte/awa/pull/302)) and **metrics off the exact-scan path** ([#330](https://github.com/hardbyte/awa/pull/330), v031) so worker health reads lane-head cursor signals instead of scanning ready rows.
...
- **`awa-seaorm` crate** ([#275](https://github.com/hardbyte/awa/pull/275)) for transactional enqueue alongside SeaORM application writes, and **registered handler futures no longer require `Sync`** ([#331](https://github.com/hardbyte/awa/pull/331)) — only `Send + 'static`.
...
- **`queue_counts_fast`** ([#289](https://github.com/hardbyte/awa/pull/289)) — index-only depth probe for high-cadence pollers (under-counts unrolled terminal rows; use `queue_counts` when exact terminal counts matter) — and **exact terminal counts without `done_entries` scans** ([#290](https://github.com/hardbyte/awa/issues/290) via [#304](https://github.com/hardbyte/awa/pull/304)/[#306](https://github.com/hardbyte/awa/pull/306)), reading folded counters when the terminal-counter trust marker is set (`awa storage rebuild-terminal-counters`; fresh installs auto-mark).
...
Coming from 0.5.x, only the storage transition (below) needs action — the 0.5 public client API is otherwise source-compatible. Within the 0.6 beta/rc series these renames also landed; an early-0.6 adopter must handle them:
