...
- **Migration compatibility pre-flights.** Additive migrations can declare a minimum runtime version; v042 requires 0.6.2, the patch that understands its compat-authority schema. `awa migrate` refuses while a fresh-heartbeat runtime reports an older or unparseable version, and names the incompatible instances. `--allow-live-runtimes` (`MigrateOptions::allow_live_runtimes`) remains the explicit override. The stricter `EXCLUSIVE_WINDOW_MIGRATIONS` mechanism remains available only for migrations with no rolling-compatible shape; v042 no longer uses it.
