...
- **Migrate pre-flight for migrations that require an exclusive window.** Migrations are flagged (an internal `EXCLUSIVE_WINDOW_MIGRATIONS` list) when a pre-migration binary cannot operate against the post-migration schema — currently **v042** ([#371](https://github.com/hardbyte/awa/issues/371)), which drops the ring-cursor columns and so needs a stop-the-world window. When a pending range crosses a flagged migration and any worker has heartbeated within the last 90 seconds, `awa migrate` refuses with a *"migration vN requires no live runtimes"* message listing the live runtimes (host, pid, heartbeat age); `--allow-live-runtimes` (`MigrateOptions::allow_live_runtimes`) overrides it, and `migrations::check_exclusive_window_preflight` runs the same check on demand. Liveness is measured by `runtime_instances.last_seen_at` staleness, **not** the `healthy` flag — a hard-killed (`kill -9`) worker leaves `healthy = true` indefinitely.

### Migrate output

- **Migrate progress output.** `awa migrate` now prints a plan line before applying (`Applying pending migrations v40..v42 (3 migrations)`) and a per-migration completion line with elapsed (`Migration v41 applied in 7752ms`), emitted as `tracing` `info!` events. Some migrations reinstall the queue-storage substrate and take several seconds each with no intermediate output; the timing lines distinguish steady progress from a hang.
