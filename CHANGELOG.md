...
- **Ring maintenance outcome fields.** `RotateOutcome::SkippedIdle` gains `generation`; `PruneOutcome::Pruned` gains `generation` and `durations: PruneDurations`. Exhaustive matches and direct construction of these public Rust enum variants must bind or ignore the new fields.
...
- **Idle prune DDL suppression and phase telemetry.** After a ring becomes idle, its cursor freezes and therefore keeps identifying the same oldest sealed generation. The maintenance leader now remembers the exact slot/generation of a committed prune and reports `already_pruned` instead of repeatedly taking `ACCESS EXCLUSIVE` and issuing `TRUNCATE`; a leadership change may conservatively repeat one prune. Successful destructive prunes publish `awa.maintenance.prune.duration` histograms for the `lock`, `truncate`, and `commit` phases, and slow-query spans include the target slot and generation.
