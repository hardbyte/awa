...
All metrics use the `awa` meter name and follow OTel semantic
conventions: dot-separated namespaces (`awa.job.*`,
`awa.dispatch.*`, …), singular nouns, UCUM units declared via
`with_unit`. Names are exported as constants from
[`awa_metrics::names`](src/lib.rs) for tests asserting against an
OTLP exporter.
