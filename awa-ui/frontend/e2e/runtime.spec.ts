...
test.describe("Runtime page", () => {
  ...
  test("renders instance and queue runtime details from API data", async ({ page }) => {
    ...
    await expect(page.getByText("worker-b")).toBeVisible();
    ...
    await expect(page.getByRole("grid", { name: "Queue runtime summary" })).toBeVisible();
    await expect(page.getByText("min 2 / w 3")).toBeVisible();
    await expect(page.getByText("5.5/s (10)")).toBeVisible();
    await expect(page.getByText("overflow held 1")).toBeVisible();
  });
});
