...
enum Commands {
    ...
    Migrate {
        ...
        pending: bool,
        /// Emit `--sql` without the surrounding transaction and advisory lock.
        ///
        /// Use this only when the consumer opens its own transaction per
        /// migration (Flyway, Liquibase, dbmate all do). Piping unwrapped SQL
        /// into `psql` applies it statement-by-statement in autocommit, so a
        /// mid-file failure leaves the schema half-migrated.
...
/// Turn a migration description into one filename component.
///
/// Deliberately minimal: spaces become `_` (as they always have) and path
/// separators become `_` because they are what actually broke extraction — a
/// description containing `/` resolved to a nested directory. Nothing else is
/// touched, so every filename this tool has already published stays
/// byte-identical and re-extracting into an existing directory cannot leave two
/// files for the same version. Only v017 and v021 change, and neither could
/// previously be written at all.
...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::Migrate {
            ...
            // `--from` / `--to` / `--version` select a *range to render*. The
            // apply path has no range: it always brings the database to
            // CURRENT_VERSION, so honouring them silently would apply far more
            // than the operator asked for (`--to 20` on a v1 database applied
            // v1..v43). Refuse instead of guessing. `--pending` is different —
            // it derives the range from the database and is the apply default.
            let applying = !sql && extract_to.is_none();
            if applying {
                let selectors = [
                    from.map(|_| "--from"),
                    to.map(|_| "--to"),
                    version.map(|_| "--version"),
                ];
                let named: Vec<&str> = selectors.into_iter().flatten().collect();
                if !named.is_empty() {
                    eprintln!(
                        "error: {} select a range to render, not a range to apply. \
                         Add --sql or --extract-to to render that range, or drop the \
                         flag to migrate the database to v{}.",
                        named.join(" / "),
            ...
            if sql {
                ...
                //
                // Wrapped in one transaction taking the same advisory lock the
                // built-in runner takes, so `awa migrate --sql | psql` is
                // atomic and serialized exactly like `awa migrate`. Without the
                // wrapper psql runs in autocommit and a failure part-way leaves
                // a half-migrated schema. `--no-transaction` opts out for
                // runners that own the transaction boundary themselves.
                //
                // The wrapper makes the failure harmless but not *visible*:
                // plain psql reports the error, fails every later statement,
                // turns the trailing COMMIT into a rollback — and still exits 0.
                // The header tells the operator to pass ON_ERROR_STOP so a
                // deploy script can actually detect the rollback.
                if !no_transaction {
                    println!(
                        "-- Generated by `awa migrate --sql`. Applies as one \
                         all-or-nothing transaction;\n\
                         -- the advisory lock serializes it against a concurrent \
                         `awa migrate`.\n\
                         -- With psql, pass -v ON_ERROR_STOP=1. Without it psql exits 0 \
                         even though\n\
                         -- the transaction rolled back, so a deploy script sees a \
                         failed migration as success.\n\
                         -- Pass --no-transaction if your migration runner opens \
                ...
                // Build every path before writing anything: a description
                // containing a path separator used to turn into a nested path
                // and abort the run part-way, leaving a partial — and silently
                // unusable — extraction on disk (v017's description contains
                // `/`, so this failed for every range reaching v017).
                let planned: Vec<(String, &String)> = selected
                    .iter()
                    .map(|(v, description, sql_text)| {
                        (
                            format!("{dir}/V{v}__{}.sql", sanitize_filename(description)),
                            sql_text,
                        )
                    })
                    .collect();
                ...
                for (path, _) in &planned {
                    if !seen.insert(path.clone()) {
                        eprintln!("error: two migrations would extract to {path}");
                        std::process::exit(1);
                    }
                }
                std::fs::create_dir_all(&dir)?;
                for (path, sql_text) in &planned {
                    std::fs::write(path, sql_text)
