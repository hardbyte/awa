...
//! transition wait on another transaction (0.6 schema patch, awa 0.6 backport of migration v046).
//!
...
/// in tests sharing the default database.
#[tokio::test]
async fn schema_patch_is_applied_once_by_migrate() {
    if setup::skip_unless_canonical("schema_patch_is_applied_once_by_migrate") {
        return;
    }
    let url = dedicated_database_url("awa_test_schema_patch").await;
    let pool = setup::pool_with_url(&url, 2).await;
    ...
    assert_eq!(pending[0].name, "wait_free_dirty_marks");
...
/// URL of a database named `name` on the same server as `DATABASE_URL`,
/// created if missing.
async fn dedicated_database_url(name: &str) -> String {
    ...
    let mut url = setup::database_url();
    let path_start = url.rfind('/').expect("DATABASE_URL has a database path");
    let query = url[path_start..]
        .find('?')
        .map(|i| url[path_start + i..].to_string());
    url.truncate(path_start + 1);
    url.push_str(name);
