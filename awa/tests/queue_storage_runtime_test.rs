...
async fn test_idle_gate_holds_rotation_open_until_sealed_slots_reclaimed() {
    ...
    // sealed slots. Under the rotation ledger (#371) this is exactly what
    // two busy rotations append: genesis (slot 0, gen 0), then (1, 1) and
    ...
    let (frozen_slot, frozen_gen) = ring_cursor(&pool, schema, "queue").await;
    let outcome = store.rotate(&pool).await.expect("repeat idle rotate");
    assert!(matches!(outcome, RotateOutcome::SkippedIdle { .. }));
    let (still_slot, still_gen) = ring_cursor(&pool, schema, "queue").await;
    assert_eq!(
