...
/// The wait_free_dirty_marks patch removed the only `TRUNCATE` from the
/// canonical maintenance path: `refresh_admin_metadata()` now deletes visible
/// dirty marks instead of truncating the mark tables. A runtime role with
/// plain DML grants must be able to run the full refresh, and the
/// trigger-maintained mark tables must be writable by the same role (triggers
/// run as the invoker).
#[tokio::test]
