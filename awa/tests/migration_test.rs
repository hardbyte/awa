...
/// #371 v041: an upgrade from a pre-v041 substrate must seed the
...
#[tokio::test]
async fn test_v041_seeds_rotation_ledger_from_legacy_cursor_and_drops_columns() {
    let _guard = acquire_migration_guard().await;
    let pool = pool().await;
    reset_schema(&pool).await;

    let schema = "awa_queue_storage_v041_ledger";
    // Drop any leftover from a prior interrupted run BEFORE migrating: the
    // v041 discovery loop reinstalls every existing queue-storage schema,
    ...
    // Downgrade the prepared (post-v041) substrate back to the pre-v041
    ...
    .execute(&pool)
    .await
    .expect("downgrade to pre-v041 substrate shape should succeed");
    ...
    // Rerun the migrations: v041's discovery loop reapplies the v023
    ...
    assert!(
        !ring_state_cursor_columns_exist(&pool, schema).await,
        "v041 must drop the legacy current_slot/generation cursor columns"
    );
    ...
    assert_eq!(
        ...
        (3, 19),
        "v041 must seed the queue ledger from the legacy cursor before dropping it"
    );
    ...
    let claimed: i64 = sqlx::query_scalar(&format!(
        "SELECT count(*)::bigint FROM {schema}.claim_ready_runtime(\
         'v041_upgrade_q'::text, 8::bigint, 0::double precision, 0::double precision)"
    ))
    .fetch_one(&pool)
    .await
    ...
    // reinstall this schema through the v041 discovery loop.
    sqlx::query(&format!("DROP SCHEMA IF EXISTS {schema} CASCADE"))
        .execute(&pool)
        .await
