...
- `storage/AwaCanonicalDirtyMarks.tla` / `storage/AwaCanonicalDirtyMarks.cfg` / `storage/AwaCanonicalDirtyMarksOnConflict.cfg`: lock-wait model for the canonical engine's admin dirty-key triggers, which run inside every `jobs_hot` / `scheduled_jobs` mutation. Plans are two application transactions that enqueue and lock an application row in opposite orders, and two worker completions whose ADR-029 follow-up enqueues touch two queues in opposite orders. The `OnConflict` config keeps the v006 `INSERT ... ON CONFLICT DO NOTHING` semantics (a keyed dirty row that blocks on another transaction's uncommitted insert) and is the expected `NoDeadlock` counterexample for the application pair. `AwaCanonicalDirtyMarksIncident.cfg` selects the 2026-09-12 production plans instead: two `Snooze` re-schedules in one queue, whose CTE fires the jobs_hot DELETE trigger and then the scheduled_jobs INSERT trigger (queue then kind, twice), plus a claim UPDATE that marks only the queue; with the drain deleting the queue row between one re-schedule's two triggers the pair acquires queue and kind in opposite orders and `NoDeadlock` is violated, exactly as the production log shows. `AwaCanonicalDirtyMarksIncidentWaitFree.cfg` and the main config model the v046 append-only marks (no unique index, plain INSERT) and pass `NoDeadlock` plus `HotPathMarksNeverWait`.
- `storage/AwaStorageTransition.tla` / `storage/AwaStorageTransition.cfg` / `storage/AwaStorageTransitionCurrentGate.cfg` / `storage/AwaStorageTransitionMigrate07Ungated.cfg`: focused model for the 0.5.x-to-0.6 storage transition control plane, extended with the 0.7 migrate gate (ADR-037). `Migrate07` only fires on a finalized or effectively fresh cluster, and `Migrate07OnlyOnQuiescedCanonical` asserts that a 0.7 migration never lands while canonical work or a canonical-only runtime is live; the ungated config is its expected-counterexample witness. The desired config matches the v014 entry gate and v040 finalization gate: a live queue-storage target is required before routing flips, while drained pre-flip runtimes may remain idle through finalization and the additive 0.7 migrations. The `CurrentGate` config remains the historical pre-v014 capability-only regression witness.
...
./correctness/run-tlc.sh storage/AwaCanonicalDirtyMarks.tla
./correctness/run-tlc.sh storage/AwaCanonicalDirtyMarks.tla storage/AwaCanonicalDirtyMarksIncidentWaitFree.cfg
...
./correctness/run-tlc.sh storage/AwaCanonicalDirtyMarks.tla storage/AwaCanonicalDirtyMarksOnConflict.cfg
./correctness/run-tlc.sh storage/AwaCanonicalDirtyMarks.tla storage/AwaCanonicalDirtyMarksIncident.cfg
