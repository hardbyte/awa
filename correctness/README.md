...
- `storage/AwaCanonicalUniqueRescue.tla` / `storage/AwaCanonicalUniqueRescue.cfg` / `storage/AwaCanonicalUniqueRescueBatchOnly.cfg`: focused model for the #388 rescue/unique-claim interaction on the canonical engine. A unique job with a pending-only `unique_states` mask is stuck running while a duplicate holds the claim; the batched rescue is blocked forever (BatchOnly config: expected `Convergence` liveness counterexample — the production wedge), while the 0.6.1 per-row fallback rescues the innocent job and cancels the superseded one (main config passes, `ClaimConsistency` holds in both).
