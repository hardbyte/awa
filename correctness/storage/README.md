...
[`AwaCanonicalUniqueRescue.tla`](./AwaCanonicalUniqueRescue.tla) models the #388 interaction between the canonical batched rescue sweeps and the `sync_job_unique_claims` trigger: a pending-only unique-states mask releases the claim while a job runs, a duplicate takes it, and the `running -> retryable` rescue transition is refused. [`AwaCanonicalUniqueRescueBatchOnly.cfg`](./AwaCanonicalUniqueRescueBatchOnly.cfg) is the expected-counterexample witness (batched sweep starves the innocent stuck job — TLC reports the `Convergence` liveness property violated); [`AwaCanonicalUniqueRescue.cfg`](./AwaCanonicalUniqueRescue.cfg) checks the 0.6.1 per-row fallback (innocent job rescued, conflicted job cancelled, `ClaimConsistency` preserved).
