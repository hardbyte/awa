...
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` (`queue_storage.rs:12260`) | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, `LOCK TABLE` `lease_claims_N`, `lease_claim_closures_N`, and `lease_claim_closure_batches_N` `IN ACCESS EXCLUSIVE MODE` with a short transaction-local `lock_timeout`, recheck not-current, anti-join check that every claim has durable closure evidence (`lease_claim_closure_batches` for compact success, explicit closures for non-success/cold paths, with durable terminal/deferred/DLQ evidence accepted as compatibility evidence for older rows), reset that slot's stale-rescue cursor, then `TRUNCATE` the three children |
...
- `rotate_claims` advances the cursor with a `FOR UPDATE` on `claim_ring_state` and a busy-check on all three child partitions of the next slot — the rotation invariant is "the slot we're flipping onto must be empty". `prune_oldest_claims` walks the full ring-state → slot-row → child bounded `ACCESS EXCLUSIVE` lock sequence and refuses to TRUNCATE while any claim in the partition lacks durable closure evidence (`PartitionTruncateSafety`).
