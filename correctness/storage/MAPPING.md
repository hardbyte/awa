...
| claim-head ready-segment cache | Nullable `{schema}.queue_claim_heads.ready_segment_slot`, `ready_segment_generation`, and `ready_segment_next_lane_seq`. This is implementation-only performance state on the same bounded lane row as `claimSeq`: it lets `claim_ready_runtime()` prune to a specific ready-slot child while the claim cursor remains inside that ready slot/generation. It is not safety evidence and is not represented as lifecycle state in TLA+; the claim function still validates the target ready row and spent evidence before emitting work. |
...
| Receipt deadline rescue | `rescue_expired_receipt_deadlines_tx`, invoked from maintenance `rescue_expired_deadlines` after the lease-side scan. Excludes claims already materialized into `leases` so the lease-side deadline path owns those. | reads `claim_ring_state` to try the oldest initialized claim slot first, then scans remaining slots with the same global rescue budget; each active claim slot has an independent bounded cursor over `(deadline_at, job_id, run_lease)` stored on `claim_ring_slots.deadline_cursor_*`, but the candidate window includes only deadlines that have already passed; anti-joins explicit `lease_claim_closures_<slot>`, compact `lease_claim_closure_batches_<slot>`, and `leases`, takes the per-`(job_id, run_lease)` advisory key with `pg_try_advisory_xact_lock`, locks expired candidates with `FOR UPDATE SKIP LOCKED`, closes them by appending `lease_claim_closures(outcome = 'deadline_expired')` and marking `lease_claims.closed_at`, and advances over closed or lease-managed claims and newly inserted deadline closures. The cursor exists so a long MVCC horizon cannot make every deadline tick repeatedly re-prove millions of already-completed receipt claims, and the expired-only window avoids repeatedly proving future-deadline claims. |
...
- `lease_claims`, `lease_claim_batches`, `lease_claim_closures`, and `lease_claim_closure_batches` are partitioned parents (`relkind = 'p'`); each has `0..N-1` children keyed by `claim_slot`. `lease_claims` and `lease_claim_closures` keep the `(claim_slot, job_id, run_lease)` identity. `lease_claim_batches` stores zero-deadline hot claims as arrays plus a derived `receipt_ranges` multirange; deadline-backed claims stay row-local in `lease_claims`. `lease_claim_closure_batches` stores arrays of immutable, globally allocated receipt ids plus a derived `int8multirange` with a GiST membership index per child, so compact-batch membership proofs use `claim_slot` plus `receipt_ranges @> receipt_id` without unnesting terminal history. Compact successful closure batches also store `ready_slot` and `ready_generation`; queue and claim prune use those columns to prove whole sealed segments by exact counts and skip conservatively when the count proof does not close the segment.
