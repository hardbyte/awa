...
- `claim_ring_slots.rescue_cursor_*` is not a separate lifecycle variable in `AwaSegmentedStorage`: it is a bounded implementation cursor over immutable `claimOpen` / `claimClosed` history. The refinement obligation is cyclic progress without dropping open claims: fresh claims may be skipped for this sweep and revisited after wrap, while stale open claims stop advancement until rescue writes a closure. `test_queue_storage_receipt_rescue_cursor_sweeps_past_fresh_claims_and_wraps` pins that behaviour in Rust.
