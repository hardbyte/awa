...
- `ClaimedEntry` carries `claim_slot: i32`; `lease_claims.receipt_id` is the exact immutable claim identity used by compact closure batches. Claim writes queue-slot-local `ready_claim_attempt_batches` range evidence before receipt rows so lost post-commit cursor advancement can be recovered without reading claim-ring parent tables. Successful compact completion groups by the originating claim slot for direct child-table claim reads and compact closure-batch writes, then writes queue-slot-routed compact terminal batches. Successful compact completion stores the claim slot on both `receipt_completion_batches` and `lease_claim_closure_batches`; non-success, rescue, cancel, and terminal-delete paths use `(claim_slot, job_id, run_lease)` to route explicit closure rows to the matching partition. A materialized receipt can live in a newer lease-ring slot than the original claim carried, so the receipt-mode lease-deleting fallback matches materialized leases by stable ready-lane and attempt identity.
