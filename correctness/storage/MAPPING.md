...
| `readySegments[seg]` state | `{schema}.ready_segments` maps committed lane sequence ranges to `(ready_slot, ready_generation)`, plus partition presence + contents (`open` ≈ current write target, `sealed` ≈ rotated out but not pruned, `pruned` ≈ TRUNCATEd). Claim uses `{schema}.ready_segments` to target the committed ready slot/generation before validating `{schema}.ready_entries`; prune deletes segment rows for the reclaimed ready slot/generation after truncating the slot's physical ready/terminal children. |
