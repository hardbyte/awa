...
| `FastComplete(w, j)` | `QueueStorage::complete_runtime_batch` short path (no attempt_state hydrate) | receipts mode: one SQL statement locks the matching `lease_claims` row, inserts `lease_claim_closures(outcome = 'completed')`, inserts compact `receipt_completion_batches` terminal history, appends a positive `queue_terminal_count_deltas` row, and clears any matching `attempt_state`. The compact path allows empty metadata plus Awa-owned queue/priority provenance metadata; custom metadata, tags, errors, or unique-key state use the general terminal path. Missed/materialized receipts fall back to the general transaction path; legacy mode: `DELETE FROM leases`; terminal reads hydrate immutable body fields through `{schema}.terminal_jobs` and the retained ready row |
...
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` (`queue_storage.rs:12260`) | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, `LOCK TABLE` `lease_claims_N` and `lease_claim_closures_N` `IN ACCESS EXCLUSIVE MODE NOWAIT`, recheck not-current, anti-join check that every claim has explicit closure evidence, with durable terminal/deferred/DLQ evidence accepted as compatibility evidence for older rows (`PartitionTruncateSafety`), reset that slot's stale-rescue cursor, then `TRUNCATE` both children |
...
| `NoLostClaim` (ADR-023) | receipts live in `claim_slot`-partitioned tables; partitions only truncate once every receipt is represented by explicit closure evidence, with durable terminal/deferred/DLQ evidence accepted for older rows, so no open claim is physically dropped |
| `ClaimOpenAndClosedDisjoint` (ADR-023) | explicit closure insertion and receipt-clearing are a single transaction for successful completion, non-success exits, cancellation, and rescue. |
...
- `Claim` now appends a receipt into the current claim segment. Successful completion (`FastComplete`, `StatefulComplete`) appends an explicit `completed` closure row and compact terminal history in the same transaction. Non-success exits (`FailToDlq`, `RetryToDeferred`, `RescueToReady`, `CancelWaitingToTerminal`, `TimeoutWaitingToDlq`, `TimeoutWaitingToReady`) append an explicit closure row in the same partition.
...
- `AwaStorageLockOrder` includes `ClaimRingStateResource`, `ClaimRingSlotResource`, `ClaimChildResource`, `ClosureChildResource`, compact receipt batch/tombstone resources, and `ClaimReceiptsPlan` / `ClaimLegacyPlan` for the two execution modes with `RowExclusive` on the appropriate child. The `*_ring_state` reads in the claim CTE are bare `SELECT`s in Rust (no `FOR SHARE`); claim is serialised against rotate via the rotator's CAS UPDATE on `(current_slot, generation)`, not via row-level locks. `CompletePlan` covers the success path with explicit closure and compact batch writes; `TerminalDeletePlan` covers cold terminal-delete materialization of explicit closures; `RotateClaimsPlan`, `PruneClaimsPlan`, `CloseReceiptPlan`, `RescueReceiptsPlan`, `EnsureRunningPlan`, and `CancelReceiptPlan` round out the receipt-plane transactions.
...
- The "currently open" set is derived at query time as `lease_claims` anti-joined with `lease_claim_closures`, with `done_entries`, compact terminal history, `deferred_jobs`, and `dlq_entries` retained as compatibility/disposition evidence where the SQL path needs it. Sites: the running-count CTE in `queue_counts_exact`, `ensure_running_leases_from_receipts_tx`, the heartbeat / progress upsert paths, `close_receipt_tx`, `rescue_stale_receipt_claims_tx`, `complete_runtime_batch`'s receipt branch, and `load_job`. None of these touch `open_receipt_claims`.
- `ClaimedEntry` carries `claim_slot: i32` so completion can lock the exact receipt claim without an extra lookup. Successful completion, non-success, rescue, cancel, and terminal-delete paths use the same triplet to route explicit closure rows to the matching partition.
