...
| `PruneReadySegment(seg)` | maintenance `prune_oldest` for the ready/terminal queue family (`queue_storage.rs:11689`) | `FOR UPDATE` on `queue_ring_state` and `queue_ring_slots[slot]`, then `LOCK TABLE ... ACCESS EXCLUSIVE NOWAIT`, recheck active rows, then `TRUNCATE`; ready-backed terminal rows, ready tombstones, and pending terminal-count deltas are reclaimed with their retained ready bodies, and Rust updates `{schema}.queue_terminal_rollups` after successful terminal prune accounting |
