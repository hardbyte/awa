...
| `claimOpen` | set of `(job_id, run_lease)` pairs with a claim row but no explicit closure row in the current claim-ring partitions. Derived at query time from `lease_claims` anti-joined with the closure table, with durable disposition rows accepted as compatibility evidence for older rows. Open receipt-only claims retain a ready row for body hydration but are excluded from `CurrentReady`; retained ready rows behind the claim cursor are also not current-ready. |
| `claimClosed` | set of `(job_id, run_lease)` pairs with a matching explicit closure row in the current claim-ring partitions. Successful receipt completion enters this set by writing a `completed` closure row in the same transaction as compact terminal history. |
...
| `RotateClaimSegments` | maintenance `QueueStorage::rotate_claims` (`queue_storage.rs:12154`), wired via `Maintenance::rotate_queue_storage_claims` at the `claim_rotate_interval` tick | `FOR UPDATE` on `claim_ring_state`, busy-check both child partitions, then `UPDATE claim_ring_state SET current_slot = next, generation = next_gen` with compare-and-swap on `(current_slot, generation)` |
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` (`queue_storage.rs:12260`) | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, `LOCK TABLE` `lease_claims_N` and `lease_claim_closures_N` `IN ACCESS EXCLUSIVE MODE NOWAIT`, recheck not-current, anti-join check that every claim has durable closure evidence (`receipt_completion_batches` for compact success, explicit closures for non-success/cold paths, with durable terminal/deferred/DLQ evidence accepted as compatibility evidence for older rows), reset that slot's stale-rescue cursor, then `TRUNCATE` both children |
| `RescueStaleReceipt(j, r)` | `rescue_stale_receipt_claims_tx` (`queue_storage.rs:8195`), invoked from maintenance `rescue_stale_heartbeats`. Excludes claims already materialized into `leases` so the lease-side rescue path owns those. The spec takes the explicit `(j, r)` so concurrent rescue / re-claim races are reachable: rescue can fire on an old attempt's `(j, r_old)` receipt while `(j, r_old + 1)` already has an open receipt in a newer partition. | reads `claim_ring_state` to try the oldest initialized claim slot first, then scans remaining slots with the same global rescue budget; each slot uses a cyclic bounded scan of `lease_claims_<slot>` from `claim_ring_slots.rescue_cursor_*`, wrapping to the start of the partition when it reaches the end; anti-join `lease_claim_closures_<slot>` and `leases`, lock stale candidates with `FOR UPDATE SKIP LOCKED`, close stragglers by appending to `lease_claim_closures` (rescue closure outcome `'rescued'`), and advance the sweep cursor across closed, lease-managed, fresh, or newly rescued claims while stopping before the first stale open claim not closed by the transaction |
| Receipt deadline rescue | `rescue_expired_receipt_deadlines_tx`, invoked from maintenance `rescue_expired_deadlines` after the lease-side scan. Excludes claims already materialized into `leases` so the lease-side deadline path owns those. | reads `claim_ring_state` to try the oldest initialized claim slot first, then scans remaining slots with the same global rescue budget; each active claim slot has an independent bounded cursor over `(deadline_at, job_id, run_lease)` stored on `claim_ring_slots.deadline_cursor_*`; anti-joins `lease_claim_closures_<slot>` and `leases`, locks expired candidates with `FOR UPDATE SKIP LOCKED`, closes them by appending `lease_claim_closures(outcome = 'deadline_expired')`, advances over closed or lease-managed claims and newly inserted deadline closures, and stops before the first open future-deadline claim. The cursor exists so a long MVCC horizon cannot make every deadline tick repeatedly re-prove millions of already-completed receipt claims. |
...
| `preparedSchemaReady` | `queue_storage_schema_ready()` / SQL checks for the queue-storage substrate needed by producers and claimers: `{schema}.job_id_seq`, `queue_ring_state`, `ready_entries`, `ready_tombstones`, `done_entries`, `receipt_completion_batches`, `receipt_completion_tombstones`, `queue_terminal_count_deltas`, `leases`, `deferred_jobs`, `lease_claims`, `lease_claim_closures`, and `claim_ready_runtime(...)` |
...
| `queueRows` | queue-storage row existence checks in `storage_abort()` across `ready_entries`, `deferred_jobs`, `leases`, `attempt_state`, `done_entries`, `receipt_completion_batches`, `receipt_completion_tombstones`, and `dlq_entries` |
...
- `AwaStorageLockOrder` includes `ClaimRingStateResource`, `ClaimRingSlotResource`, `ClaimChildResource`, `ClosureChildResource`, compact receipt batch/tombstone resources, and `ClaimReceiptsPlan` / `ClaimLegacyPlan` for the two execution modes with `RowExclusive` on the appropriate child. The `*_ring_state` reads in the claim CTE are bare `SELECT`s in Rust (no `FOR SHARE`); claim is serialised against rotate via the rotator's CAS UPDATE on `(current_slot, generation)`, not via row-level locks. `CompletePlan` covers the compact success path with closure-evidence reads and compact batch writes; `TerminalDeletePlan` covers cold terminal-delete materialization of explicit closures; `RotateClaimsPlan`, `PruneClaimsPlan`, `CloseReceiptPlan`, `RescueReceiptsPlan`, `EnsureRunningPlan`, and `CancelReceiptPlan` round out the receipt-plane transactions.
