...
| `PruneLeaseSegment` | `QueueStorage::prune_oldest_leases` (`queue_storage.rs:12021`) | `TRUNCATE` the selected `leases_N` child only after active-row checks |
...
The spec's PruneLeaseSegment transition also captures the analogous concern on `prune_oldest` (for ready partitions) at `queue_storage.rs:11689`.
