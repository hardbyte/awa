...
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, proof that every row-local claim or compact claim-batch item in the sealed slot has durable closure evidence, then `LOCK TABLE` `lease_claims_N`, `lease_claim_batches_N`, `lease_claim_closures_N`, and `lease_claim_closure_batches_N` `IN ACCESS EXCLUSIVE MODE` with a short transaction-local `lock_timeout`, recheck not-current, recheck the open-claim proof, reset that slot's stale-rescue cursor, then `TRUNCATE` the four children. The proof first compares exact child-row counts (`lease_claims` rows plus `lease_claim_batches.claimed_count` totals versus explicit closures plus compact `closed_count` totals) and only falls back to the full anti-join when counts do not prove closure. The fallback anti-join accepts `lease_claim_closure_batches` for compact success, explicit closures for non-success/cold paths, and durable terminal/deferred/DLQ evidence as compatibility evidence for older rows. |
...
- `ClaimedEntry` carries `claim_slot: i32` and `receipt_id: Option<i64>`; `receipt_id` is the exact immutable claim identity used by compact closure batches whether the receipt lives in `lease_claims` or `lease_claim_batches`. Claim writes queue-slot-local `ready_claim_attempt_batches` range evidence before receipt evidence so lost post-commit cursor advancement can be recovered without reading claim-ring parent tables. Successful compact completion groups by the originating claim slot for direct child-table claim reads and compact closure-batch writes, validates the claim by `receipt_id`, then writes queue-slot-routed compact terminal batches. Successful compact completion stores the claim slot on both `receipt_completion_batches` and `lease_claim_closure_batches`; non-success, rescue, cancel, and terminal-delete paths use `(claim_slot, job_id, run_lease)` to route explicit closure rows to the matching partition. A materialized receipt can live in a newer lease-ring slot than the original claim carried, so the receipt-mode lease-deleting fallback matches materialized leases by stable ready-lane and attempt identity.
