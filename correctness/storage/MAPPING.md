...
| `readySegments[seg]` state | `{schema}.ready_segments` maps committed lane sequence ranges to `(ready_slot, ready_generation)`, plus partition presence + contents (`open` ≈ current write target, `sealed` ≈ rotated out but not pruned, `pruned` ≈ TRUNCATEd). Segment construction splits at `run_at` boundaries so the compact row carries the exact lane-head timestamp used for claim-time priority aging. Claim uses `{schema}.ready_segments` to target the committed ready slot/generation before validating `{schema}.ready_entries`; prune deletes segment rows for the reclaimed ready slot/generation after truncating the slot's physical ready/terminal children. |
...
| `preparedSchemaReady` | `queue_storage_schema_ready()` / SQL checks for the queue-storage substrate needed by producers and claimers: `{schema}.job_id_seq`, `queue_ring_state`, `ready_entries`, `ready_tombstones`, `ready_segments`, `done_entries`, `receipt_completion_batches`, `receipt_completion_tombstones`, `queue_terminal_count_deltas`, `leases`, `deferred_jobs`, `lease_claims`, `lease_claim_closures`, `lease_claim_closure_batches` with its `receipt_ranges` membership column, and `claim_ready_runtime(...)` |
