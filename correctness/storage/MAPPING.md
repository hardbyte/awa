...
| `heartbeatFresh` | `heartbeat_at` on the lease row + the maintenance cutoff (see `rescue_stale_heartbeats` in `queue_storage.rs:10618`) |
...
| `ProgressFlush(j)` | `QueueStorage::flush_progress` (`queue_storage.rs:9781`) | `UPDATE attempt_state SET progress = ... WHERE job_id = $1 AND run_lease = $2` guarded by running/waiting_external state |
| `ParkToWaiting(w, j)` | `QueueStorage::enter_callback_wait` (`queue_storage.rs:9188`) from executor `WaitForCallback` | `UPDATE leases SET state = 'waiting_external', heartbeat_at = NULL, deadline_at = NULL`; attempt_state is preserved |
| `ResumeWaitingToRunning(j)` | `QueueStorage::complete_external(..., resume = true)` (`queue_storage.rs:9392`) | `UPDATE leases SET state = 'running', callback_id = NULL, callback_timeout_at = NULL, heartbeat_at = clock_timestamp()` plus callback result upsert in `attempt_state` |
...
| `FailToDlq(w, j)` | `QueueStorage::fail_to_dlq` (`queue_storage.rs:10126`) / `fail_terminal` (`:10081`) via executor terminal failure path | `DELETE FROM leases`, `DELETE FROM attempt_state`, `INSERT INTO dlq_entries` in one tx |
| `RetryToDeferred(w, j)` | `QueueStorage::retry_after` (`queue_storage.rs:9947`) / `snooze` (`:9997`) on `JobError::RetryAfter` / `Snooze` | `DELETE FROM leases`, `INSERT INTO deferred_jobs` |
...
| `MoveFailedToDlq(j)` | `QueueStorage::move_failed_to_dlq` (`queue_storage.rs:10178`); admin entry in `awa-model/src/dlq.rs:170` | hydrate from retained ready body, `DELETE FROM done_entries`, append a negative `queue_terminal_count_deltas` row, and `INSERT INTO dlq_entries` guarded by state=failed. The retained ready row remains until queue prune. |
| `RetryFromDlq(j)` | `QueueStorage::retry_from_dlq` (`queue_storage.rs:10290`) | CTE: `DELETE FROM dlq_entries RETURNING ...` + `INSERT INTO ready_entries ...` (Rust resets `run_lease` to 0 because the new claim row will live in a different `claim_slot` partition; the spec keeps `run_lease` monotonic per-job since it abstracts away `claim_slot` from the receipt key); unique-conflict handled by `sync_unique_claim` |
...
| `PruneReadySegment(seg)` | maintenance `prune_oldest` for the ready/terminal queue family (`queue_storage.rs`) | `FOR UPDATE` on `queue_ring_state` and `queue_ring_slots[slot]`, prove the sealed-slot skip gates first (active leases, receipt claims that still lack durable closure evidence, and retained ready rows still at or ahead of their lane's claim cursor), then `LOCK TABLE ... ACCESS EXCLUSIVE` with a short transaction-local `lock_timeout` and `TRUNCATE`; this guard prevents queue prune from removing terminal history before claim closure evidence exists or removing ready/tombstone cursor evidence before the lane cursor has passed it. Ready-backed terminal rows, compact completion batches, compact tombstones, ready tombstones, and pending terminal-count deltas are reclaimed with their retained ready bodies; `{schema}.ready_segments` rows for the pruned ready slot/generation are deleted in the same transaction; Rust updates `{schema}.queue_terminal_rollups` after successful terminal prune accounting |
| `PruneLeaseSegment` | `QueueStorage::prune_oldest_leases` (`queue_storage.rs:12021`) | `FOR UPDATE` on `lease_ring_state` and `lease_ring_slots[slot]`, then `LOCK TABLE ... ACCESS EXCLUSIVE` with a short transaction-local `lock_timeout`, recheck active rows, then `TRUNCATE` the selected `leases_N` child |
| `RotateClaimSegments` | maintenance `QueueStorage::rotate_claims` (`queue_storage.rs:12154`), wired via `Maintenance::rotate_queue_storage_claims` at the `claim_rotate_interval` tick | `FOR UPDATE` on `claim_ring_state`, busy-check the three child partitions, then `UPDATE claim_ring_state SET current_slot = next, generation = next_gen` with compare-and-swap on `(current_slot, generation)` |
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` (`queue_storage.rs:12260`) | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, proof that every claim in the sealed slot has durable closure evidence, then `LOCK TABLE` `lease_claims_N`, `lease_claim_closures_N`, and `lease_claim_closure_batches_N` `IN ACCESS EXCLUSIVE MODE` with a short transaction-local `lock_timeout`, recheck not-current, reset that slot's stale-rescue cursor, then `TRUNCATE` the three children. The proof first compares exact child-row counts (`lease_claims` rows versus explicit closures plus compact `closed_count` totals) and only falls back to the full anti-join when counts do not prove closure. The fallback anti-join accepts `lease_claim_closure_batches` for compact success, explicit closures for non-success/cold paths, and durable terminal/deferred/DLQ evidence as compatibility evidence for older rows. |
...
The spec's PruneReadySegment transition also captures the analogous concern on `prune_oldest` (for ready partitions) at `queue_storage.rs:11689`.
