...
| `FastComplete(w, j)` | `QueueStorage::complete_runtime_batch` short path (no attempt_state hydrate) | receipts mode: one SQL statement locks the matching `lease_claims` row, inserts `lease_claim_closures(outcome = 'completed')`, inserts compact `receipt_completion_batches` terminal history, appends a positive `queue_terminal_count_deltas` row, and clears any matching `attempt_state`. Missed/materialized receipts fall back to the general transaction path; legacy mode: `DELETE FROM leases`; terminal reads hydrate immutable body fields through `{schema}.terminal_jobs` and the retained ready row |
