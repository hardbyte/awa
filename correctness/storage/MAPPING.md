...
| `claimSegmentOf[<<j, r>>]` | the `claim_slot` column on the `(job_id, run_lease)` claim row in `{schema}.lease_claims` (ADR-023); explicit closure rows in `{schema}.lease_claim_closures` and compact closure rows in `{schema}.lease_claim_closure_batches` share the same `claim_slot`. The spec keys claim bookkeeping by `(job, run_lease)` rather than by `job` alone so that an old attempt's receipt survives the next claim into a newer partition — Rust's `(claim_slot, job_id, run_lease)` triplet is the actual partition-side unique key, while compact closure batches use the immutable `receipt_id` assigned on the claim row. The model abstracts the `claim_slot` half away into `claimSegmentOf`. |
...
| `FastComplete(w, j)` | `QueueStorage::complete_runtime_batch` short path (no attempt_state hydrate) | receipts mode: the transaction first takes per-`(job_id, run_lease)` advisory locks in deterministic key order, then one SQL statement locks the matching `lease_claims` rows, skips claims that already have materialized `leases` rows, inserts compact `lease_claim_closure_batches` evidence keyed by `receipt_id`, inserts compact `receipt_completion_batches` terminal history, appends positive `queue_terminal_count_deltas` rows, and clears any matching `attempt_state`. Known-key receipt materialization, progress, explicit close, and compact completion paths use the same advisory key before row locks so a same-attempt transition observes the committed winner before deciding whether the receipt is still open. The compact path allows empty metadata plus Awa-owned queue/priority provenance metadata; custom metadata, tags, errors, unique-key state, or materialized lease state use the general terminal path. Missed receipts fall back to the general transaction path; legacy mode: `DELETE FROM leases`; terminal reads hydrate immutable body fields through `{schema}.terminal_jobs` and the retained ready row |
...
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` (`queue_storage.rs:12260`) | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, anti-join check that every claim in the sealed slot has durable closure evidence (`lease_claim_closure_batches` for compact success, explicit closures for non-success/cold paths, with durable terminal/deferred/DLQ evidence accepted as compatibility evidence for older rows), `LOCK TABLE` `lease_claims_N`, `lease_claim_closures_N`, and `lease_claim_closure_batches_N` `IN ACCESS EXCLUSIVE MODE` with a short transaction-local `lock_timeout`, recheck not-current, reset that slot's stale-rescue cursor, then `TRUNCATE` the three children |
