...
| `claimSegmentOf[<<j, r>>]` | the `claim_slot` column on the `(job_id, run_lease)` claim row in `{schema}.lease_claims` (ADR-023); explicit closure rows in `{schema}.lease_claim_closures` and compact closure rows in `{schema}.lease_claim_closure_batches` share the same `claim_slot`. The spec keys claim bookkeeping by `(job, run_lease)` rather than by `job` alone so that an old attempt's receipt survives the next claim into a newer partition — Rust's `(claim_slot, job_id, run_lease)` triplet is the actual partition-side unique key, while compact closure batches use the immutable `receipt_id` assigned on the claim row. Compact closure batches also carry `ready_slot` / `ready_generation` for implementation-level prune count proofs; the model abstracts those columns away because they do not change which attempts are open or closed. |
| `claimOpen` | set of `(job_id, run_lease)` pairs with a claim row but no durable closure evidence in the current claim-ring partitions. Derived at query time from `lease_claims` anti-joined with explicit closures, compact closure batches, and compatibility disposition rows. Open receipt-only claims retain a ready row for body hydration but are excluded from `CurrentReady`; retained ready rows behind the claim cursor are also not current-ready. |
...
| `FastComplete(w, j)` | `QueueStorage::complete_runtime_batch` short path (no attempt_state hydrate) | receipts mode: the transaction first takes per-`(job_id, run_lease)` advisory locks in deterministic key order, then one SQL statement validates the matching `lease_claims` row by exact `(claim_slot, job_id, run_lease, receipt_id)`, skips claims that have already materialized `leases` rows, inserts compact `lease_claim_closure_batches` evidence keyed by `receipt_id` and annotated with the ready slot/generation, inserts compact `receipt_completion_batches` terminal history, and clears any matching `attempt_state`. It does not `FOR UPDATE` the claim row on compact success; same-attempt mutual exclusion comes from the advisory key shared by compact completion, known-key receipt materialization, progress, explicit close, and receipt rescue. Compact completions write no terminal-count delta rows; exact counts read retained compact batches directly and subtract compact tombstones. The compact path allows empty metadata plus Awa-owned queue/priority provenance metadata; custom metadata, tags, errors, unique-key state, or materialized lease state use the general terminal path. Missed receipts fall back to the general transaction path; if a receipt claim materialized into `leases` after the original claim, the fallback deletes that lease by stable `(ready_slot, ready_generation, job_id, queue, priority, enqueue_shard, lane_seq, run_lease)` identity rather than by the claim's old lease slot. Legacy mode: `DELETE FROM leases`; terminal reads hydrate immutable body fields through `{schema}.terminal_jobs` and the retained ready row |
...
| `PruneClaimSegment(seg)` | `QueueStorage::prune_oldest_claims` | `FOR UPDATE` on `claim_ring_state`, `FOR UPDATE` on `claim_ring_slots[slot]`, proof that every claim in the sealed slot has durable closure evidence, then `LOCK TABLE` `lease_claims_N`, `lease_claim_closures_N`, and `lease_claim_closure_batches_N` `IN ACCESS EXCLUSIVE MODE` with a short transaction-local `lock_timeout`, recheck not-current, recheck the open-claim proof, reset that slot's stale-rescue cursor, then `TRUNCATE` the three children. The proof first compares exact child-row counts (`lease_claims` rows versus explicit closures plus compact `closed_count` totals) and only falls back to the full anti-join when counts do not prove closure. The fallback anti-join accepts `lease_claim_closure_batches` for compact success, explicit closures for non-success/cold paths, and durable terminal/deferred/DLQ evidence as compatibility evidence for older rows. |
...
| `preparedSchemaReady` | `queue_storage_schema_ready()` / SQL checks for the queue-storage substrate needed by producers and claimers: `{schema}.job_id_seq`, `{schema}.lease_claim_receipt_id_seq`, `queue_ring_state`, `ready_entries`, `ready_tombstones`, `ready_segments`, `done_entries`, `receipt_completion_batches`, `receipt_completion_tombstones`, `queue_terminal_count_deltas`, `leases`, `deferred_jobs`, `lease_claims`, `lease_claim_closures`, `lease_claim_closure_batches` with its `receipt_ranges` membership column, the ready-segment cache columns on `queue_claim_heads`, and `claim_ready_runtime(...)` |
...
| `queueRows` | queue-storage row existence checks in `storage_abort()` across `ready_entries`, `ready_tombstones`, `ready_segments`, `deferred_jobs`, `leases`, `attempt_state`, `done_entries`, `receipt_completion_batches`, `receipt_completion_tombstones`, `lease_claims`, `lease_claim_closures`, `lease_claim_closure_batches`, and `dlq_entries` |
...
- `lease_claims`, `lease_claim_closures`, and `lease_claim_closure_batches` are partitioned parents (`relkind = 'p'`); `lease_claims_0..N-1`, `lease_claim_closures_0..N-1`, and `lease_claim_closure_batches_0..N-1` are the children. `lease_claims` and `lease_claim_closures` keep the `(claim_slot, job_id, run_lease)` identity; `lease_claim_closure_batches` stores arrays of immutable, globally allocated `lease_claims.receipt_id` values plus a derived `int8multirange` with a GiST membership index per child, so compact-batch membership proofs use `receipt_ranges @> receipt_id` directly rather than rechecking `claim_slot`. Compact successful batches also store `ready_slot` and `ready_generation`; queue and claim prune use those columns to prove whole sealed segments by exact counts before falling back to per-claim membership proofs.
- The "currently open" set is derived at query time as `lease_claims` anti-joined with durable closure evidence: explicit `lease_claim_closures`, compact `lease_claim_closure_batches`, `done_entries`, `deferred_jobs`, and `dlq_entries` where the SQL path needs compatibility/disposition evidence. Sites: the running-count CTE in `queue_counts_exact`, `ensure_running_leases_from_receipts_tx`, the heartbeat / progress upsert paths, `close_receipt_tx`, `rescue_stale_receipt_claims_tx`, `complete_runtime_batch`'s receipt branch, and `load_job`. None of these touch `open_receipt_claims`.
- `ClaimedEntry` carries `claim_slot: i32` and `receipt_id: Option<i64>`; `lease_claims.receipt_id` is the exact immutable claim identity used by compact closure batches. Claim writes queue-slot-local `ready_claim_attempt_batches` range evidence before receipt rows so lost post-commit cursor advancement can be recovered without reading claim-ring parent tables. Successful compact completion groups by the originating claim slot for direct child-table claim reads and compact closure-batch writes, validates the claim row by `receipt_id`, then writes queue-slot-routed compact terminal batches. Successful compact completion stores the claim slot on both `receipt_completion_batches` and `lease_claim_closure_batches`; non-success, rescue, cancel, and terminal-delete paths use `(claim_slot, job_id, run_lease)` to route explicit closure rows to the matching partition. A materialized receipt can live in a newer lease-ring slot than the original claim carried, so the receipt-mode lease-deleting fallback matches materialized leases by stable ready-lane and attempt identity.
