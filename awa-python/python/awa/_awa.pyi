...
class RetryFailedResult:
    @property
    def jobs(self) -> list[Job[dict[str, Any]]]: ...
    @property
    def matched(self) -> int: ...
    @property
    def pruned_failed_count(self) -> int | None: ...
...
class Client:
    ...
    async def cron_reconciliation_plan(self, owner: str) -> str: ...
    def cron_reconciliation_plan_sync(self, owner: str) -> str: ...
    async def cron_owner_action(self, action_json: str, *, actor: str = "python", apply: bool = False) -> str: ...
    def cron_owner_action_sync(self, action_json: str, *, actor: str = "python", apply: bool = False) -> str: ...
    def __init__(
...
# Callback receiver contract (see awa.callback_contract for the friendly
# wrapper module, and ADR-027 / docs/callback-receivers.md for usage).
CALLBACK_SIGNATURE_HEADER: str
CALLBACK_DEFAULT_HEARTBEAT_TIMEOUT_SECS: float
CALLBACK_DEFAULT_PATH_PREFIX: str

def sign_callback(secret: bytes, callback_id: str) -> str: ...
def verify_callback(secret: bytes, callback_id: str, signature: str) -> bool: ...
def build_callback_url(
    base: str, prefix: str, callback_id: str, action: str
) -> str: ...
...
class StorageNotFinalized(AwaError): ...
class LiveRuntimesRequireExclusiveWindow(AwaError): ...
class RuntimeVersionFloorNotMet(AwaError): ...
