...
def main() -> None:
    ...
    p_migrate.add_argument(
        ...
        action="store_true",
        dest="no_transaction",
        help="Emit --sql without the surrounding transaction and advisory lock. Use only when the consuming migration runner opens its own transaction per migration.",
    )
...
async def _migrate(args: argparse.Namespace) -> None:
    ...
    if args.no_transaction and not args.sql:
        print("--no-transaction only applies to --sql output.", file=sys.stderr)
        sys.exit(1)
    ...
    # Mirror the Rust CLI: the rendered SQL is wrapped in one transaction that
    # takes the runner's advisory lock, so `python -m awa migrate --sql | psql`
    # is atomic and serialized exactly like `awa migrate`. Without the wrapper
    # psql applies it in autocommit and a mid-stream failure leaves the schema
    # half-migrated.
    wrap = not args.no_transaction
    if wrap:
        print(
            "-- Generated by `awa migrate --sql`. Applies as one all-or-nothing transaction;\n"
            "-- the advisory lock serializes it against a concurrent `awa migrate`.\n"
            "-- With psql, pass -v ON_ERROR_STOP=1. Without it psql exits 0 even though\n"
            "-- the transaction rolled back, so a deploy script sees a failed migration as success.\n"
